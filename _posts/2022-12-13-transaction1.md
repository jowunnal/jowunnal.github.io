...
즉, Commit 연산이라는 행위는 데이터베이스의 연산로직이 디스크에 쓰여질 것 임을 보장하는 행위이다. 마치 우리가 월세계약을 하기위해 계약서에 도장을 찍고, 구청에가서 전입신고를 하는것 처럼 말이다.
...
그리고 장애가 발생할때 'LOG' 를 참고하여 디스크에 데이터를 write하게 된다.
...
원자성은 뗄레야 뗄수없는 더이상 쪼갤수없는 특징을 원자성이라고 한다. 즉 All or Nothing으로써 하나의 작업은 모두 수행되던지, 아니면 둘다 수행되지말던지 둘중에 하나가 되어야한다.
...
따라서, commit이 수행되기 전에 장애가 발생하면 이모든 연산은 수행되지 않는 Nothing이 되어야만 한다. 마치 계약서에 도장을 찍기전에 수상한점을 발생해서 엎는격인 것이다.(물론 여기서는 Exception과 같은 장애가 발생했다는 전제)
...
지연갱신에서 위상황은 commit이 수행되기 전이므로 로그에 기록되어져 있지 않은 상태다. 즉 장애가 발생하면 그냥 없던일로 취급하고 '무시' 하고 로그의 내용만 버리면 된다.
