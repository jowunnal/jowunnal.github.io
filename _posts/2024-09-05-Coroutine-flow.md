...
하지만, 이렇게 실시간으로 활성화 상태를 유지하는 채널은 그자체로 비용이 발생합니다. 또, 보통 외부에 데이터를 요청하고 응답받는 네트워크 통신에서는 지속적으로 활성화 되지 않고 요청을 했을 때 발생한 데이터를 응답받아 처리합니다. 이런 특징을 Cold Stream 이라고 하며, 채널과 같이 실시간으로 활성화(Live)되어 있는 것을 Hot Stream 이라고 합니다. 채널은 기본적으로 Hot 하고, 네트워크 통신과 같이 요청했을 때만 활성화 되는 Cold 를 지원하기 위해 Flow 가 제공되었습니다.
...
Hot Flow 는 채널처럼 collect 와 같은 최종 연산이 없어도 활성화 되어 있습니다. 따라서, 언제든지 값이 방출(emit) 되면 replyCache 만큼 최신의 데이터를 가지고 있다가 구독자가 collect 시점에 데이터를 소비할 수 있습니다. 또한, 채널과 달리 구독자가 구독을 시작한 뒤로 어떤 문제로 인해 데이터를 받지못할 때 extraBufferCapacity 만큼 데이터를 오버플로우 전략에 따라 관리하면서 들고 있을 수 있습니다. 
