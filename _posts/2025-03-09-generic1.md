...
결론적으로, "제네릭은 타입에 관계없이 순수한 기능과 속성에 대한 일반화 개념이고, 객체지향 프로그래밍 내에서 타입을 생성 및 호출 시점에 결정 시킴으로써 발생하는 타입 안정성 문제를 해결하기 위해 제네릭 클래스와 인터페이스의 경우 변성이라는 성질을 사용한다" 고 정리할 수 있을 것 같습니다.

그렇다면, 클래스나 인터페이스는 변성을 통해 타입 안정성을 확보할 수 있지만, 제네릭 함수와 같이 변성을 둘 수 없는 경우 어떻게 타입 안정성을 높일 수 있을까요? 
...
그러면서 IDEA 는 __Change type arugments to <*>__ 라는 것을 하도록 추천합니다. <*> 가 의미하는 게 뭘까요? 바로 __스타 프로젝션__ 입니다. 
