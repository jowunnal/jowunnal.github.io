...
마지막으로 제네릭 파라미터 타입 앞에 in 키워드를 명시하면 반공변성을 나타냅니다. 이는 제네릭 타입간의 관계가 타입 파라미터간의 관계에서 __역전되는 것__ 을 말합니다. 즉, 부모-자식 관계가 역전되어 MutableList\<in Number> 를 함수 인자로 요구하는 경우, MutableList\<Int>를 전달할 수 없습니다. 
...
위의 오류는 선언부(클래스)에서 읽기 전용(in)이라 명시한 상태에서, 사용부(함수)에서 반대의 변성(out)을 지정하여 충돌이 발생했다는 컴파일 에러 입니다. 이러한 오류가 발생한 이유는 List의 경우 변경 가능한 연산자와 기능이 제공되지 않는 읽기 전용 이고, 따라서 선언 지점에서 out 키워드를 명시하였는데, 사용 지점에서 프로젝션하여 충돌이 발생됬기 때문입니다.
...
```kotlin
fun inputData(input: Number) {
    val destination: MutableList<Int> = mutableListOf()
    destination.add(input) // --> 예외 발생: Number cannot be casting to Int
}
```
...
결론적으로, "제네릭은 타입에 관계없이 순수한 기능과 속성에 대한 일반화 개념이고, 객체지향 프로그래밍 내에서 타입을 생성 및 호출 시점에 결정 시킴으로써 발생하는 타입 안정성 문제를 해결하기 위해 변성이라는 성질을 사용한다" 고 이해할 수 있을 것 같습니다.
