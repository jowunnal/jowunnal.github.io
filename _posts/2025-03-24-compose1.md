...
이후, UI 에 변경사항이 생기게 되면 Invalidiation(무효화) 를 하게 되고, 이는 Recomposition 이 트리거 합니다. Recomposition 은 Composable 함수를 재 실행하여, 현재 상태를 기반으로 생성된 변경사항들을 특정 시간에 적용하는 과정들을 실행하게 됩니다. 이 과정에서 모든 Composable 함수들을 매번 Recomposition 하여 방출하게 되면, 짧은 시간이내에 여러번 호출될 경우 버벅임과 같은 성능 문제를 야기할 수 있습니다.
