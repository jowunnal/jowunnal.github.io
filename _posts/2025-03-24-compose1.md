...
이번 포스팅의 주제는 Compose Compiler 입니다. Compose Compiler 의 주된 역할은 __Compose Runtime 이 필요로 하는 정보들을 IR 변환에 직접 개입하여 코드를 생성 혹은 변환하여 제공 하는 것__ 입니다. 개발자가 작성한 Composable 함수들은 Runtime 이 되어서야 모두 실행되어 UI 의 인메모리 표현을 생성하는 Composition 과정이 실행됩니다.(이를 "방출" 과정이라고 합니다.) 이렇게 생성된 UI 의 인메모리 표현(트리)는 Slot table 이라는 자료구조로 관리됩니다. Slot table 은 [Gap buffer](https://en.wikipedia.org/wiki/Gap_buffer "link") 라는 자료구조를 기반으로 만들어 졌습니다. 해당 자료구조의 정확한 동작 원리가 궁금하시다면 [개발자가 직접 작성한 아티클](https://medium.com/androiddevelopers/under-the-hood-of-jetpack-compose-part-2-of-2-37b2c20c6cdd "link") 을 참고하시면 도움이 될 것 같습니다.

Composition 이 끝나면, 방출과정을 통해 생성된 인메모리 표현(트리)을 기반으로 생성할 뷰의 크기와 위치를 결정한 뒤, 그리게 됩니다.

이후, UI 에 변경사항이 생기게 되면 Invalidiation(무효화) 를 하게 되고, 이는 Recomposition 이 트리거 합니다. Recomposition 은 Composable 함수를 재 실행하여, 현재 상태를 기반으로 생성된 변경사항들을 특정 시간에 적용하는 과정들을 실행하게 됩니다. 이 과정에서 모든 Composable 함수들을 매번 Recomposition 하여 방출하게 되면 애니메이션과 같이 짧은 시간이내에 여러번 호출될 경우 버벅임과 같은 성능 문제를 야기할 수 있습니다.
...
간혹 보면, "Recomposition 이 일어나지 않도록" 또는 "Recomposition 이 빈번하게 발생하지 않도록" 에 초점을 맞추는 경우를 보곤 합니다. 하지만 Recomposition 은 궁극적으로 상태의 변화에 의해 Compose Runtime 이 트리거 하게 됩니다. 즉, 개발자가 이것을 직접 트리거 할 수 없으며 "Recomposition 의 제어" 에 초점을 맞추는 것은 조금 이상할지도 모릅니다.

오히려, Compose Compiler 와 Compose Runtime 의 내부 동작을 학습해 둔다면, "__상태를 어떻게 관리__ 할 것인가" 와 Recomposition 을 건너뛸 수 있도록 __Skippable 하게 만드는 최적화 전략__ 들을 알 수 있으며, 이를 토대로 어떻게 동작할지에 대해 예측할 수 있고 성능 개선을 위한 최적화 방법들을 적용할 수 있으므로 이 두가지 측면에 초점을 맞추는게 중요하다고 생각합니다.
...
## @Composable
개발자가 Compose 로 UI를 선언하기 위해서는 간단하게 함수에 @Composable 어노테이션을 작성하기만 하면 됩니다.
...
Compose 는 선언적 프로그래밍을 실현하는 함수형 프로그래밍 방식의 UI Toolkit 입니다. 함수형 프로그래밍에서 는 주요한 특징 중 하나로 __순수함수__ 가 있습니다. 이는 함수의 같은 입력 매개변수가 주어졌을 때, 여러번 거듭 실행하더라도 같은 출력(결과)이 만들어져야 한다는 특성 입니다.(이를 다른 말로 "멱등성" 이라고도 합니다.)

```kotlin
fun add(a: Int, b: Int): Int = a + b // a = 1, b = 2 일때 몇번을 실행하더라도 결과값은 3 입니다.
```

Composable 함수도 마찬가지입니다. UI 에 대한 변경사항이 발생한다면, 이는 Invalidiation(무효화) 을 트리거 하게 됩니다. Invalidiation 은 변경사항이 있는 Composable 함수에 대해서만 선택적으로 Recomposition 을 트리거하고, 나머지에 대해서는 Recomposition 을 건너뛸 수 있어 그대로 재 사용 함으로써 성능적 이점을 만들어 줄 수 있습니다.
...
내부적으로는 Composition 이 발생할 때, Composable 함수를 읽어들여 방출하게 되면 Slot table 내에 상태(State)나 Composable 함수와 같은 모든 추적 가능한 관련 정보를 __기억(저장)__ 해둔 뒤, 변경 사항이 없는 경우 __기억된 값을 그대로 재사용하여__ 성능을 개선합니다. Compose Runtime 이 이러한 동작을 가능하도록 코드를 변환하고 생성하는 것이 Compose Compiler 의 주된 역할입니다.

순수함수적 특징을 검증하기 위해 Compose 에서는 __안정성(Stability)__ 이라는 규칙을 이용합니다. Compose Runtime 은 안정성을 기반으로 동일한 입력에 대해 동일한 출력이 발생할 것을 예상하고 해당 부분의 Recomposition 을 건너뛸(skip) 수 있습니다. 따라서, 안정적이지 않은 경우 변경사항이 발생할 경우 매번 Recomposition 을 수행하게 됩니다.  
...
- 값을 캡처하지 않거나, 안정적인 값을 캡처하는 람다식은 안정적으로 간주됩니다.(Kotlin 2.0.20 버전 부터는 [Strong Skipping Mode](https://developer.android.com/develop/ui/compose/performance/stability/strongskipping?hl=ko "link") 가 Default 로 적용되어 ___안정적이지 않은 값을 캡처하는 람다식도 안정적으로 변환됩니다.___ )
...
또한 안정적으로 간주되지 않는 타입을 입력 매개변수로 갖는다면, 해당 Composable 의 Recomposition 을 건너뛸 수 없고 매번 동작하게 되므로 성능 문제가 발생할 여지를 갖게 된다는 점을 기억해두시면 좋겠습니다. 이러한 매커니즘을 활용하여 성능 개선에 초점을 맞춘 [코드랩](https://developer.android.com/codelabs/jetpack-compose-performance?hl=ko#0 "link")을 보시는것도 활용 측면에서의 이해를 높이는데 도움이 될 수 있을것 같습니다.
...
Compose Compiler 는 모든 Composable 함수에 대해 인메모리 표현의 Root 에서 Composer 라는 인스턴스를 생성하여 최하위 노드(Leaf) 까지 전파합니다. 이를 입력 매개변수에 삽입하는 형태로 모든 Composable 함수에 대해 합성하여 대체 합니다.
...
Compose Compiler 는 모든 람다식을 안정적으로 추론한다고 안정성 부분에서 확인했었습니다. 하지만 람다식이 값을 캡처하는 경우 그 값의 변화에 따라 멱등성을 만족하지 못할 수 있습니다. 따라서 람다식에 대해 안정적으로 추론하기 위해서 람다식을 최적화 하는 과정을 거칩니다.

#### Composable 이 아닌 람다식

Composable 이 아닌 람다식이면서, 값을 캡처하지 않는 람다의 경우 Side Effect 가 존재하지 않으므로(외부상태의 변화에 따라 결과를 예측할 수 없는 상황) Compose Compiler 는 해당 람다식을 Object 로 변환하여 싱글톤으로 모델링한 뒤, 이를 재 사용하게 됩니다.
...
Composable 람다식의 경우 Composable 이 아닌 람다식과는 달리 Slot table 에 저장되기 때문에 같은 방식인 Kotlin 의 object 로 변환될 수 없습니다. 하지만 최적화의 맥락은 비슷하게 remember 가 아닌 State 로 wrapping 되는 형태로 합성됩니다.

Composable 람다식은 composableLamda(composer, key, shouldBeTracked ...) 이라는 팩토리 함수로 mapping 됩니다. composer 는 호출하는 Composable 함수에서 전달받은 Composer 인스턴스가 그대로 전달되며, key 는 해당 람다식을 slot table 에 저장하기 위한 정체성(인메모리 표현에서의 해당 노드에 대한 식별 가능한 key 값)을 나타내며, shouldBeTracked 의 경우 해당 람다식이 Compose Runtime 에 의해 추적 가능한지의 여부를 나타냅니다. 람다식이 만약 값을 캡처하지 않는다면 Composable 이 아닌 람다식과 마찬가지로 싱글톤으로 모델링되어 재 사용 되기 때문에 추적될 필요가 없고, 이 때 해당 람다식이 추적될 수 없음을 표시하는 용도로 사용됩니다.

값을 캡처하는 Composable 람다식의 경우 Compose Runtime 에 의해 추적되며, MutableState와 유사하게 구현된 방식에 따라 State<@Composable () -> Unit> 로 구현된 것과 유사하게 변형됩니다. 이를 통해  SnapShot State 객체를 생성하여 궁극적으로 __Donut hole Skipping__ 이라고 불리는 최적화 방법으로 이용됩니다. 

Donut hole Skipping 은 Composable 람다식이 업데이트되는 과정에서 상위 Composable 에서 하위 Composable 로 전달될 때, 실제로 해당 람다식이 읽혀지는 하위 Composable 에서만 Recomposition 이 일어나는 최적화 방법을 의미합니다. State Hoisting 과 같이 상태 자체가 하향으로 전달되는 빈도가 높지만, 실제로 해당 상태를 최하위에서만 읽고, 중간이나 그 위에서는 읽지 않는 경우 적합하게 사용되는 __지연읽기__ 의 맥락과 같습니다.

## 비교 전파

Composable 함수의 입력 매개변수들의 변경사항을 감지하여 해당 함수를 선택적으로 Recomposition 할 수 있다고 언급했었습니다. Compose Compiler 는 Composable 함수에 대해 Composer 인스턴스를 생성하여 전달하는 것 처럼, 함수의 입력 매개변수들에 대해 정적(Static)인 경우 __비트 마스크__ 한 결과인  Changed 라는 Int 타입의 변수를 생성하여 전달합니다.
...
a 라는 값은 static 변수로 10이라는 값이 Compile time 에 결정되고, runtime 에 변경되지 않습니다. 이러한 경우 Compose Compiler 는 입력 매개변수들에 대해 비트마스크 한 결과를 Changed 라는 변수로 이용하여, equals() 함수로의 비교를 생략할 수 있습니다. Recomposition 이 짧은 시간 이내에 여러번 트리거 되었을 때, equals() 의 결과를 비교하지 않고 비트 단위의 연산 방식인 비트마스크의 결과를 비교하여 성능상 이점을 만듭니다.

Composer 인스턴스를 생성하여 상위 Composable 에서 최하위 Composable 까지 전달하는 것과 마찬가지로 Changed 값 역시 하위로 전파됩니다. 하위 Composable 이 상위 Composable 로 부터 매개변수를 전달 받는 경우 해당 값이 변경되었는지, 정적인지, 안정적인지 또는 불안정적인지 와 같은 정보를 하위로 전달함으로써 하위 Composable 의 Recomposition 을 건너뛰는데 사용할 수 있습니다. 이러한 최적화 방법을 위해 해당 비트마스크를 하위로 전파하는 것을 "비교 전파" 라고 합니다.
...
- Restartable Group : Composable 이 상태를 읽는 경우, Recomposition 이 발생했을 때 해당 Composable 을 재시작하기 위한 용도로 해당 Composable 을 이 그룹으로 wrapping 합니다. 만약, 읽고있는 상태가 없다면, 재시작할 이유가 없으므로 Recomposition 이 발생했을 때 해당 Composable 을 그냥 그대로 사용합니다. 해당 그룹으로 감싸졌다면,  runtime이 재구성시에 변경된 상태를 기반으로 어떻게 해당 Composable 을 재시작 할 수 있는지에 대해 가르치는 역할을 담당합니다.
- Replaceable Group : 이 그룹으로 감싸진 경우 RememberedValue(기억된값) 으로 저장한 후, 변경사항이 발생하면 해당 값을 update 하여, 기억된 값을 반환하는 형태로 재 사용 합니다. 해당 그룹의 대표적인 사용사례로 조건부 논리(if) 의 분기문을 적용 받거나, Composable 람다식 최적화, @NonRestartableComposable 어노테이션이 작성된 Composable 의 경우 ReplaceableGroup 으로 wrapping 됩니다. 
